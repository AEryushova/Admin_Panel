...
public class TestHelper {
    ...
    public static Timestamp generateDate(String period) {
        ...
        return switch (period) {
            case "previous" -> Timestamp.valueOf(LocalDate.now().minusMonths(monthOffset).atStartOfDay());
            case "future" -> Timestamp.valueOf(LocalDate.now().plusMonths(monthOffset).atStartOfDay());
            case "current" -> Timestamp.valueOf(LocalDateTime.now());
            case "yesterday" -> Timestamp.valueOf(LocalDateTime.now().minusDays(1));
    ...
    public static String getDate(String period, int monthOffset, int dayOffset, String datePattern) {
        LocalDate date = switch (period) {
            case "current" -> LocalDate.now().plusDays(dayOffset);
            case "previous" -> LocalDate.now().minusMonths(monthOffset).plusDays(dayOffset);
            case "future" -> LocalDate.now().plusMonths(monthOffset).plusDays(dayOffset);
            default -> throw new IllegalArgumentException("Invalid period: " + period);
        };
    ...
    public static String getDay(String period, String dayType, int monthOffset, int dayOffset) {
        LocalDate date = switch (period) {
            case "current" -> LocalDate.now();
            case "previous" -> LocalDate.now().minusMonths(monthOffset);
            case "future" -> LocalDate.now().plusMonths(monthOffset);
            default -> throw new IllegalArgumentException("Invalid period: " + period);
        };
        if (dayType != null) {
