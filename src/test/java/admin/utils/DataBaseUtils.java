...
public class DataBaseUtils {
    ...
    private static QueryRunner getQueryRunner(String datasourceKey) {
    ...
    public static void clearAllFeedback() {
        ...
        getQueryRunner("cab_lab_db").update(connection, "DELETE FROM feedbacks_employees");
    }

    @SneakyThrows
    public static void addFeedback() {
        var connection = getConnection("cab_lab_db");
        getQueryRunner("cab_lab_db").execute(connection, "INSERT INTO feedbacks_employees (employees_id,author,content,created_at,updated_at,is_published ) VALUES ('787a79cc-daa6-11ee-ae4c-572334db0f4a','Федоров Ф. Ф.','Это тестовый коммент','2024-05-27 12:28:49.000', '2024-05-27 12:28:49.000', 'false'");
    }

    @SneakyThrows
    public static void clearAllBugReports() {
        var connection = getConnection("cab_lab_db");
        getQueryRunner("cab_lab_db").execute(connection, "DELETE FROM bug_reports");
    }

    @SneakyThrows
    public static void clearAllFaq() {
        var connection = getConnection("cab_lab_db");
        getQueryRunner("cab_lab_db").execute(connection, "DELETE FROM faq");
    }
    ...
    public static Feedback selectFeedback() {
        ...
        var connection = getConnection("cab_lab_db");
        return getQueryRunner("cab_lab_db").query(connection, selectFeedbackRequest, new BeanHandler<>(Feedback.class));
    }
    ...
    public static BugReports selecBugReports() {
        ...
        var connection = getConnection("cab_lab_db");
        return getQueryRunner("cab_lab_db").query(connection, selectBugReports, new BeanHandler<>(BugReports.class));
    }
    ...
    public static Faq selectFaq() {
        ...
        var connection = getConnection("cab_lab_db");
        return getQueryRunner("cab_lab_db").query(connection, selectFaq, new BeanHandler<>(Faq.class));
    }
}